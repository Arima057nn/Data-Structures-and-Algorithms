...
int nb_internalNode(tree T)
{
  if (isEmpty(T))
    ...
    return 1 + nb_internalNode(T->right) + nb_internalNode(T->left);
}
...
void insertNode(element x, tree *T)
{
  ...
  else if ( ((*T)->(data.sdt - '\0') > (x.sdt - '\0')) insertNode(x,&(*T)->left);
  else if ( ((*T)->(data.sdt - '\0') <= (x.sdt - '\0')) insertNode(x,&(*T)->right);
}