...
void displayElement(element e){
  //printf("%s\t%d\n",e.ID,e.Score);  
  printf("%-10s %-25s %-15d %-5d %-5d %-5d\n",e.ID,e.Name,e.money,e.TP,e.DT,e.MM);  
}
...
void OutPutFile(FILE *ptr,tree T){
  if(T != NULL){
    ...
    fprintf(ptr,"%-20s %-20s %-20d %-15d %-15d %-15d\n",T->data.ID,T->data.Name,T->data.money,T->data.TP,T->data.DT,T->data.MM);
