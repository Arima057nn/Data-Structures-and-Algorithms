...
void displayElement(element e){
  printf("%-10d %-20s  %-10d %-10d %-10d\n",e.Id,e.Tendoibong,e.Sobanthang,e.Sobanthua,e.Diem);  
}
...
void DeleteNode(element e, tree *T){
  if(*T != NULL){
    if(compare2((*T)->data,e) == 1) DeleteNode(e,&(*T)->left);
    else if(compare2((*T)->data,e) == -1) DeleteNode(e,&(*T)->right);
...
tree Searching(element x, tree T){
  if(T == NULL || compare2(T->data,x) == 0) return T;
  else if(compare2(T->data,x) == 1) return Searching(x,T->right);
  else if(compare2(T->data,x) == -1) return Searching(x,T->left);
}
void SEARCHING(int *n,element e,tree T){
  if(T != NULL){
    SEARCHING(n,e,T->left);
    if(compare2(T->data,e) == 0){
      *n = 1;
