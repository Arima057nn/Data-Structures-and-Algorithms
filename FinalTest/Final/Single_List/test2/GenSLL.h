...
int compare3(element a,element b){
    if(a.id == b.id)
        return 0;
    if (a.id > b.id)
        return 1;
...
void displayNode(node *p)
{
    ...
    {
        printf("%-10d %-10s %-10d\n",p->data.id,p->data.CPU,p->data.RAM);
    }
}
/* element input(){
    element new;
...
void updateRAM(node **root, element e)
{
    node *p;
    int i = 0;
    for (p = *root; p != NULL; p = p->next)
    {
        if (compare3(p->data, e) == 0)
        {
            do{
                printf("Nhap RAM dung dinh dang de bai: ");
                scanf("%d",&p->data.RAM);
            }while( dinhDang2(p->data.RAM) !=  1);
...
void updateCPU(int *n,node **root, element e)
{
    char s[10];
    node *p;
    int i = 0;
    for (p = *root; p != NULL; p = p->next)
    {
        ...
        {
            do{
                printf("Nhap CPU dung dinh dang de bai: ");
                scanf("%s%*c",s);
                if( strcmp(s,"STOP") == 0){
                    *n = 1;
                    return;
                }
                else strcpy(p->data.CPU,s);
            }while( dinhDang1(p->data.CPU) !=  1);           
            return ;
        }
    }
