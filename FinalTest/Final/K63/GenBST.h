...
int nb_leaf(tree T)
{
  if (T == NULL)
    return 0;
  if (rightChild(T) == NULL || leftChild(T) == NULL)
    return 1;
  else
...
int nb_RightChild(tree T)
{
  if (T == NULL)
    return 0;
  if (T->right == NULL)
    return nb_RightChild(T->left);
  else
    return 1 + nb_RightChild(T->right) + nb_RightChild(T->left);
}
...
void insertNode(element e, tree *T)
{
  ...
  else if (compare((*T)->data, e) == 1)
    return insertNode(e, &(*T)->left);
  else if (compare((*T)->data, e) == -1)
    return insertNode(e, &(*T)->right);
}
...
void DeleteNode(element e, tree *T)
{
  if (*T != NULL)
  {
    if (compare((*T)->data, e) == 1)
      DeleteNode(e, &(*T)->left);
    else if (compare((*T)->data, e) == -1)
      DeleteNode(e, &(*T)->right);
...
{
  if (T == NULL || compare(T->data, x) == 0)
    return T;
  else if (compare(T->data, x) == 1)
    return Searching(x, T->right);
  else if (compare(T->data, x) == -1)
    return Searching(x, T->left);
}
...
void SEARCHING2(int *n, element e, tree T)
{
  if (T != NULL)
  {
    SEARCHING2(n, e, T->left);
    if (compare(T->data, e) == 0)
    {
      *n = 1;
      printf("%s ",T->data.nghia);
    }
    SEARCHING2(n, e, T->right);
  }
